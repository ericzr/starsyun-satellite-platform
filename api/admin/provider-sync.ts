...
function input(body: unknown) {
  ...
  return { providerId, mode: mode as SyncMode, bbox: bbox?.map(Number) as [number, number, number, number] | undefined, datetime: typeof value.datetime === 'string' ? value.datetime : undefined, cloudCoverMax: value.cloudCoverMax == null ? undefined : Number(value.cloudCoverMax), limit: value.limit == null ? undefined : Number(value.limit) };
}
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    try {
      ...
      await rest(`provider_sync_runs?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify({ status: 'succeeded', result_summary: result.summary, records_seen: result.recordsSeen, records_upserted: result.recordsUpserted, finished_at: finishedAt }) });
