...
4. 如果当前语言没有经过审核的翻译，该行政区不进入当前语言的选择器和搜索结果。

英文模式可以使用规范的 `name_en`，因为它就是英文界面所需的规范名称；其他语言绝不把英文或数据源的未知 `local` 名称当作翻译回退。这样选择器永远不会出现占位文案或混合语言。缺失项由后台覆盖率审计报告记录，继续通过 GeoNames 或其他审核过的标准名称源补齐后才会显示。
