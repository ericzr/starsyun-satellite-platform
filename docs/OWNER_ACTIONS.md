...
1. **企业支付宝**：确认企业账号是否已签约“电脑网站支付”，创建生产应用，登记回调地址 `https://starsyun.com/api/webhooks/alipay`，并确认退款权限、结算主体和币种。应用私钥只存放服务器。未完成前，继续使用对公转账加管理员人工核销，不能展示“支付宝可支付”。
...
3. **中国行政区权威数据**：确认可商用且可追溯的中国 ADM1-ADM3 数据授权来源，并提供版本、许可、下载/API 入口与更新频率。当前 geoBoundaries 中国低级别数据不能直接上线，见 [全球行政区数据方案](./GLOBAL_ADMIN_BOUNDARIES.md)。
